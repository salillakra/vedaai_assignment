...
const createBadge = (text: string, bgColor: string, fgColor: string = colors.fg.white) => {
...
export const logger = {
  ...
  http: (method: string, path: string, status: number, duration: number) => {
    const statusColor = status < 300 ? colors.fg.green : status < 400 ? colors.fg.yellow : colors.fg.red;
    ...
    console.log(`${badge} ${time} ${methodCol} ${pathCol} ${statusCol} ${durationCol}`);
  },
};
